resource "aws_instance" "jenkins" {
  ...
  provisioner "local-exec" {
    command = <<EOT
	  ...
	  ansible-playbook -u ${var.ansible_user} --private-key ${var.private_key} -i java install-java-mvn.yaml
    EOT
  }
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      ansible-playbook -u ${var.ansible_user} --private-key ${var.private_key} -i jenkins install-jenkins-docker.yaml -e "hub_username=DockerHubUName hub_password=DockerHubPswrd"
    EOT
  }
